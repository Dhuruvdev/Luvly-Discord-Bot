...
export default {
  ...
  async execute(message, args, client) {
    ...
    if (!amount || amount < 1)
      return message.reply({ flags: CV2, components: [luvContainer('>  specify an amount — `u gamble 100` or `u gamble all` ✦', errorRow)] });
    if (amount > wallet)
      return message.reply({ flags: CV2, components: [luvContainer(`>  not enough in wallet! you have **${fmt(wallet)}** ✦`, errorRow)] });
    if (amount > 100_000)
      return message.reply({ flags: CV2, components: [luvContainer('>  max bet is 100,000 luv per spin ✦', errorRow)] });
