...
const JOBS = [
  ...
  { title: 'poet',           emoji: '🖊️', flavor: 'sold one poem for exactly enough' },
...
const COOLDOWN_MS = 30 * 60 * 1_000; // 30 min
...
export default {
  ...
  async execute(message, args, client) {
    ...
    const now     = Date.now();
    const lastWork = u.lastWork ?? 0;
    const waited  = now - lastWork;
    ...
    if (waited < COOLDOWN_MS) {
      ...
      const embed  = luvEmbed(COLORS.neutral)
        ...
        .setDescription(`you're still recovering from your last shift.\n\n⏳ back in **${leftM} min**`)
    ...
    // Base: 20–60 luv × market mood × inflation (wages track inflation)
    ...
    const { saveUserEco } = await import('../../utils/economy.js').catch(() => ({ saveUserEco: () => {} }));

    // Manually update lastWork
    ...
    const embed = luvEmbed(COLORS.success)
      ...
      .addFields(
        ...
        { name: '📊 market',   value: `${eco.marketTrend} (×${mult.toFixed(2)})`, inline: true },
      )
      .setFooter(footer(client));
