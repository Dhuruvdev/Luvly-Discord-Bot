  ...
  async execute(message, args, client) {
    ...
    if (!sub || (sub !== 'dep' && sub !== 'deposit' && sub !== 'with' && sub !== 'withdraw')) {
      ...
      const text =
        `**﹕ⵌ┆  Luvly Bank ꩜ .**\n\n` +
        (frozen
          ? ` **account frozen** — your loan exceeds 2× your bank balance. repay debt to unfreeze.\n\n`
    ...
    if (!amount || amount < 1) {
      return message.reply({ flags: CV2, components: [luvContainer('>  specify amount — `u bank dep 500` or `u bank dep all` ✦', backRow)] });
    }

    if (sub === 'dep' || sub === 'deposit') {
      const result = deposit(userId, amount);
      if (!result.success) {
        return message.reply({ flags: CV2, components: [luvContainer(`>  not enough in wallet! you have **${fmt(wallet)}** ✦`, backRow)] });
      }
      const text =
        `**﹕ⵌ┆  Deposited ꩜ .**\n\n` +
    ...
    if (!result.success) {
      ...
      return message.reply({ flags: CV2, components: [luvContainer(`>  not enough in bank! you have **${fmt(bank)}** ✦`, backRow)] });
    }
