...
export default {
  ...
  async execute(message, args, client) {
    ...
    if (sub === 'repay' || sub === 'pay') {
      if (loan <= 0)
        return message.reply({ flags: CV2, components: [luvContainer('> ⚠️ you have no outstanding loan ✦', backRow)] });

      const raw    = args[1];
      const amount = raw === 'all' ? wallet : parseInt(raw ?? '', 10);
      if (!amount || amount < 1)
        return message.reply({ flags: CV2, components: [luvContainer('> ⚠️ specify amount — `u loan repay all` or `u loan repay 500` ✦', backRow)] });

      const result = repayLoan(userId, amount);
      if (!result.success) {
        if (result.reason === 'no_loan')
          return message.reply({ flags: CV2, components: [luvContainer('> ⚠️ no loan to repay ✦', backRow)] });
        return message.reply({ flags: CV2, components: [luvContainer(`> ⚠️ not enough in wallet! you have **${fmt(result.balance)}** ✦`, backRow)] });
      }
    ...
    if (loan > 0)
      return message.reply({ flags: CV2, components: [luvContainer(`> ⚠️ you already have an active loan of **${fmt(loan)}** — repay it first ✦`, backRow)] });

    const amount = parseInt(sub, 10);
    if (amount < 100)      return message.reply({ flags: CV2, components: [luvContainer('> ⚠️ minimum loan is 100 luv ✦', backRow)] });
    if (amount > MAX_LOAN) return message.reply({ flags: CV2, components: [luvContainer(`> ⚠️ max loan is **${fmt(MAX_LOAN)}** ✦`, backRow)] });

    const result = takeLoan(userId, amount);
    if (!result.success)
      return message.reply({ flags: CV2, components: [luvContainer(`> ⚠️ couldn't process loan — ${result.reason} ✦`, backRow)] });
