...
export default {
  ...
  async execute(message, args, client) {
    ...
    if (!target)
      return message.reply({ flags: CV2, components: [luvContainer('> ⚠️ mention someone to send luv to ✦', errRow)] });
    if (target.id === message.author.id)
      return message.reply({ flags: CV2, components: [luvContainer("> ⚠️ you can't send luv to yourself 💀", errRow)] });
    ...
    if (!amount || amount < 1)
      return message.reply({ flags: CV2, components: [luvContainer('> ⚠️ specify a valid amount — `u give @user 100` ✦', errRow)] });
    if (amount > 1_000_000)
      return message.reply({ flags: CV2, components: [luvContainer('> ⚠️ max single transfer is 1,000,000 luv ✦', errRow)] });

    const result = transfer(message.author.id, target.id, amount);
    if (!result.success)
      return message.reply({ flags: CV2, components: [luvContainer(`> ⚠️ not enough in wallet! you have **${fmt(result.balance)}** ✦`, errRow)] });
